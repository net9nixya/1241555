...
export type Profile = {
  ...
  // Цвет ника — HEX-строка (например "#4da3ff"), null — обычный цвет.
  // Покупается за Counter Coin в магазине (отдельный товар на каждый
  // цвет), выбирается из купленных в настройках. Виден всем, кто
  // открывает профиль (свой профиль, топы, поиск).
  nickColor: string | null;
  // Ключи цветов ника, которыми владеет игрок ("pink"|"blue"|"green"|"red")
  // — из этого списка строится выбор цвета в настройках.
  nickColorInventory: string[];
  ...
  devVerified?: boolean;
  // Покупная верификация — третий, отдельный от verified и devVerified
  // флаг. Покупается ИГРОКОМ САМ за Counter Coin в магазине мини-аппа
  // (10000 монет). Как и devVerified, показывается только в мини-аппе, в
  // самом боте тегом не отображается. Рисуется отдельной (красной) иконкой,
  // порядок иконок у ника: dev → paid → обычная verified.
